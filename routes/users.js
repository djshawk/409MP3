...
router.post('/', async function(req, res) {
  try {
    ...
    const pendingTasks = Array.isArray(req.body.pendingTasks) ? req.body.pendingTasks
                         : (req.body.pendingTasks ? [].concat(req.body.pendingTasks) : []);

    if (!name || !email)
      return res.status(400).json({ message: 'name and email are required', data: null });

    const exists = await User.findOne({ email });
    if (exists) return res.status(400).json({ message: 'email already in use', data: null });
...
// GET /api/users/:id  (supports select= or filter= projection)
...
router.put('/:id', async function(req, res) {
  ...
  try {
    ...
    if (conflict) {
      await session.abortTransaction(); session.endSession();
      return res.status(400).json({ message: 'email already in use', data: null });
    }
    ...
    // Assign tasks from pendingTasks — block if already assigned elsewhere
    const tasksToAssign = await Task.find({ _id: { $in: Array.from(newSet) } }).session(session);
    for (const t of tasksToAssign) {
      if (t.assignedUser && t.assignedUser !== String(user._id)) {
        await session.abortTransaction(); session.endSession();
        return res.status(400).json({ message: 'task already assigned to another user', data: String(t._id) });
      }
      if (!t.completed) {
